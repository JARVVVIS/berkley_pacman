    ...
    while not v.isEmpty():
      ...
      for node in list(children):
        if node[0] not in visited:
          ...
          v.push((node[0],actions+[node[1]])) ##node 0 holds the xy; node[1] holds the action required to reach this node
    return []    
...
def breadthFirstSearch(problem):
    ...
    visited = []
    visited.append(problem.getStartState())
    v.append((problem.getStartState(),[]))
    while len(v)!=0:
      (current_node,actions) = v.pop(0)
      ...
      for node in list(children):
        if node[0] not in visited:
          visited.append(node[0])
          v.append((node[0],actions+[node[1]]))
    return []
